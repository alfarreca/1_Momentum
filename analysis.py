...
def calculate_momentum(hist: pd.DataFrame):
    """Replicates/cleans logic from the original script for a single ticker window."""
...
def filter_results(df, min_score: int, exchange: str | None = None):
    if df is None or df.empty:
        return df
    if exchange and exchange != "All":
        mask = (df["Momentum_Score"] >= min_score) & (df["Exchange"] == exchange)
    else:
        mask = (df["Momentum_Score"] >= min_score)
